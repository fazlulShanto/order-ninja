...
function OrderSummary({ setTotal ,next,shipping}) {
    ...
    const getSingleCartItem = async (id) => {
    ...
    useEffect(() => {
        ...
        const getProduct = async () => {
            try {
                ...
                const cost = await getPathaoCost({weight : _itemWeight,city:shipping.city,zone:shipping.zone});
                setCost(cost);
        ...
        <div style={{width:'100%',display:'flex',justifyContent:'center'}}>
          <div style={{width:'50vw'}}>
            ...
            {finalData.map((v) => (
                ...
                >
                    <Col key={Math.random()} span={8}> {v.name} </Col>
                    <Col key={Math.random()} span={8}> {v.quantity} </Col>
